...
std::string HuffmanTree::decodeMessage(std::string codeMessage){

	string decodedMessage = "";

	Node* currNode = rootNode;
	for(unsigned int i = 0; i < codeMessage.length()+1; i++){

		if(currNode == NULL) break;

		if(currNode == NYTNode){
			//if we reached the NYTNode, then is ASCII/UTF-8 for next 8 bits
			//add new message to tree
			//also case for initial value

			std::string aSCIIInBinary = codeMessage.substr(i, 8);
			int newMessage = getASCII(aSCIIInBinary);
			encodeMessageCode(newMessage);

			decodedMessage += newMessage;

			//we need to skip next eight bits
			i += 8;

			//reset the currNode to rootNode
			currNode = rootNode;
		}

		//existing symbol
		//we've just traversed a path
		if (currNode->isLeaf()){
			decodedMessage += (int)currNode->messageCode;
			
			//increment the message weight
			encodeMessageCode(currNode->messageCode);

			//reset the currNode to rootNode
			currNode = rootNode;
		}

		//break if we have incremented outside array range (need to do this because we need to check last node traveral)
		if(i >= codeMessage.length()) break;

		//move down tree based on message binary
		if (codeMessage[i] == '0') {
			currNode = currNode->leftChild;
		} else if  (codeMessage[i] == '1') {
...
int main(int argc, char* argv[]){
	...
	*/
	//input
	for (string inputLine; getline(std::cin, inputLine);) {
		
		HuffmanTree* huffmanTree = new HuffmanTree();

		cout << huffmanTree->decodeMessage(inputLine)  <<endl;
